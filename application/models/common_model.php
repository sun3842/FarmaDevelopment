...
class Common_Model extends CI_Model
{
	...
	public function db_login_authentication()
	{
		...
		$query = $this->db->get_where('admin_user', array('admin_user_name' => $posted_login_user_name,'admin_user_active'=>1));
		...
		$target_login_password_value=$login_table_row['admin_user_password_hash_value'];
		$username=$login_table_row['admin_user_name'];
		$user_type=$login_table_row['admin_user_ref_user_type_id'];
		$user_id=$login_table_row['admin_user_id'];
		
		$pharmacy_id=$login_table_row['admin_user_ref_pharmacy_id'];
		...
		if($this->validate_password($posted_login_password, $target_login_password_value))
		{
			...
			$this->session->set_userdata('pharmacy_id', $pharmacy_id);
